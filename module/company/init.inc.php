...
if($is_write){
	...
	if(!empty($_GET['rewrite'])){
	 //判断是否存在city
		$city_pos=strripos($_GET['rewrite'], 'city'); 
	}
	
	if($city_pos===false){
		//首页没有city要city=1
	    if($file=='homepage' || empty($_COOKIE['zz_city'])){
	        $city=1; 
	    }else{
	       $city=intval($_COOKIE['zz_city']); 
	    } 
	}else{
		$city_data=substr($_GET['rewrite'],$city_pos);
    	$city=intval(substr($city_data,strripos($city_data, '_')+1)); 
	...
	if(empty($_GET['city'])){
	    ...
	    //首页没有city要city=1
	    if(empty($_GET['file']) || empty($_COOKIE['zz_city'])){
	        $city=1; 
	    }else{
	       $city=intval($_COOKIE['zz_city']); 
	    } 
	}else{
		$city_data=trim($_GET['city']);
    	$city=intval(substr($city_data,strripos($city_data, '_')+1)); 
	}
}
...
setcookie("zz_city", $city);
 
//获取单条信息
if($city>1){
    $city_get= $db->get_one("SELECT id,name,pname FROM `{$DT_PRE}area3` WHERE `id`=$city");
    
    if(empty($city_name)){
        $city_name='';
        $city=1;
    }else{
        $city_name=$city_get['name'];
    }
}else{
    $city_name='';
}
//获取下属城市
$citys = array(0=>array('id'=>1,'name'=>'总站','pname'=>'main'));
//组装city_url
$city_url='';
$city_url1='';
if(!empty($city_get)){
    $citys[]=$city_get;
    $city_url='&city='.$city_get['pname'].'_'.$city_get['id'];
    $city_url1=$city_url;
    if($is_write){
    	//重写规则
    	///city-nanchong_334.html
		$city_url1='city-'.$city_get['pname'].'_'.$city_get['id'].'.html';
		$COM[linkurl]=userurl($username, 'file=homepage', $domain);
    }  
}

$result = $db->query("SELECT id,name,pname FROM `{$DT_PRE}area3` WHERE `fid`=$city ORDER BY `listorder` asc");
while($r = $db->fetch_array($result)) {
    $citys[] = $r;
}
//地区分站关键字处理
if($city>1 && !empty($HOME['seoweb_keywords'])){
    $head_keywords=$HOME['seoweb_keywords'];
}
$arr_words=explode(',', $head_keywords);
//追加在标题的前3个keywords
$zz_keywords='';
foreach($arr_words as $k=>$v){
    $arr_words[$k]=$city_name.$v;
    if($k<2){
        $zz_keywords.=','.$arr_words[$k];
    } 
}
