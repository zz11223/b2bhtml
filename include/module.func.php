<?php
/*
	[CHENGXIN315 B2B System] Copyright (c) 2008-2018 www.chengxin315.com
...
function get_albums($item, $type = 0) {
	$imgs = array();
	//为无图的产品补图，现取消
	if($item['thumb'] && preg_match("/^[a-z0-9\-\.\:\/]{30,}$/i", $item['thumb'])){
	    $imgs[] = $item['thumb'];
	}
	if($item['thumb1'] && preg_match("/^[a-z0-9\-\.\:\/]{30,}$/i", $item['thumb1'])){
	    $imgs[] = $item['thumb1'] ;
	}
	if($item['thumb2'] && preg_match("/^[a-z0-9\-\.\:\/]{30,}$/i", $item['thumb2'])){
	    $imgs[] = $item['thumb2'];
	}
	if(empty($imgs)){
		if($type == 0) {
			$imgs[] = DT_SKIN.'image/nopic60.gif';
		}else{
			$imgs[] = DT_SKIN.'image/nopic320.gif';
		}
	}
	return $imgs;
	/*
