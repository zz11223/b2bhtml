...
if($action == 'company') {
	...
	$lists = array();
	if($items) {
		$result = $db->query("SELECT * FROM {$table} WHERE $condition ORDER  ORDER BY level desc,addtime DESC  LIMIT $offset,$pagesize");
...
if($itemid) {
	...
	$lists = array();
	if($items) {
		// ORDER BY level desc,edittime DESC 
		$result = $db->query("SELECT * FROM {$table} WHERE $condition ORDER BY level desc,addtime DESC  LIMIT $offset,$pagesize");
		while($r = $db->fetch_array($result)) {
			$r['alt'] = $r['title'];
			$r['title'] = set_style($r['title'], $r['style']);
